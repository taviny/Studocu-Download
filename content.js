(function () {
  ...
  function cropImage(dataUrl, cropX, cropY, cropW, cropH) {
  ...
  async function capturePageElement(pageEl, dpr) {
    ...
    if (fullH <= viewH) {
      ...
      return await cropImage(raw, cropX, Math.max(0, Math.floor(curRect.top * dpr)), cropW, cropH);
    }
  ...
  async function captureContinuousPages(contentEl, dpr) {
    ...
    for (let i = 0; i < numSlices; i++) {
      ...
      const cropped = await cropImage(raw, cropX, 0, cropW, Math.floor(sliceH * dpr));
  ...
  async function smartExport() {
    ...
    if (capturedPages.length === 0) {
      return { success: false, error: 'Không chụp được trang nào.' };
    }
    ...
    for (let i = 0; i < capturedPages.length; i++) {
      ...
      pdf.addImage(img.dataUrl, 'PNG', posX, posY, imgW, imgH, undefined, 'FAST');

      pdf.setFontSize(12);
      pdf.setTextColor(215, 215, 215);
      pdf.text('@vny', 8, pageHeight - 8);
    }
